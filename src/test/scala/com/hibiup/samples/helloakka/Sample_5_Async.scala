...
class Example_5_Couroutine extends FlatSpec{
    /**
      * 们都知道在 RX 编程中，消息驱动扮演了重要的角色，它是任务驱动的重要手段之一。于是很多程序员就因此误认为掌
    ...
    /** *
      * 以上这段代码我们用 akka 系统实现了消息驱动，通过一个 actor 得到问候语。先不纠结于此架构对业务来说有多大的
      * 价值，让我们仅仅纯粹从架构的角度来思考一下它的价值有多大？很遗憾，答案是这个实现毫无价值，连“几乎”都配不上。
      * (akka 的分布式附加值不是我们本次考察的重点）。为什么呢？为了简化问题，我们现在抛开 akka，仅以纯粹的多线程来
      * 实现这个例子中的任务模型。
      */
    ...
    /**
      ...
      * 最多只是一个普通的多线程架构。更甚至，这个多线程架构相对于传统单线程架构来说，也毫无优势。我们完全可以将任
      * 务线程和主线程合并，只用一个主线程既生成问候语，又打印问候语，在架构的价值上也是完全等价的，因为在工作线程
      * 执行的过程中，主线程是空闲的，在这期间它完全可以不需要工作线程（如果不考虑分布的话）取而代之。难道不是么？
      *
      * 为了进一步说明这个问题，我们以打乒乓球为例。在乒乓球比赛中，选手A和选手B相当于两个线程，互相击球就相当于互
      * 相调用，当选手A将球击打给B后，任务就交给了选手B，在B回球的过程中，选手A其实处于等待状态。这在现实比赛中习
      * 以为常，但是在电脑的CPU中，这是极大的资源浪费，如果选手A的速度足够快，他完全可以回家吃个饭再回来，没有必
      * 要专门等待对手的回球。如果他够敬业的话，他甚至他可以跑到球桌的对面去回击自己发出的球，那么他一个人（线程）
      * 就可以完成这场比赛，根本不需要选手B。这也就是我们上面两个例子阐述的问题。所以以上都不是真正的异步架构。为
      * 此我们有必要来澄清一下有关异步的几个误区：
      *
      * 误区一：消息驱动是异步编程重要的任务驱动手段，但不是异步架构的充分条件。
      * 误区二：真正的异步架构其实并不刻意追求多线程，它追求的是最有效的CPU利用率。
      *
      * 那么怎样才是最有效的CPU利用率呢？让我们还是以乒乓球比赛为例。但是我们现在要改一下规则，现在我们增加一个称
      * 为“选手池”的东西，它用来存放没有上场的选手，其次、我们也不再区分你我阵营，所有的选手都来自这个“选手池”，
      * 还有，我们也不再规定开球的选手只能在他的发球侧打比赛，甚至不要求他在开出球之后必须继续在球桌边等待对方的回
      * 球。比赛的评判标准仅仅以乒乓球能够在球桌两边被不断被击打为唯一评判标准。也就是说，我们关注的是业务能够被持
      * 续执行下去，而不关注由多少线程，以什么样的方式，在何时何地执行下去。那么我们将可能会看到这样一种比赛场景，
      * 选手A发出球后就离开了比赛现场回到“选手池”等待下一个调配去了，而选手B在回球后也离开了现场打电话去了，当球被
      * 打回到发球侧的时候，选手C恰逢其时地出现了，他替已经下场的选手A将球击回了另一边，而球在另一边即将落地前，选
      * 手A及时回到了球桌的另一边又将球击了回来……。
      *
      ...
      * 比赛场上！他们绝不会同时出现在同一个场地中去等待任何其他人击球！也就是说，虽然这是个多线程的程序，但是每个
      * 业务主线都相当于单线程！但是他们能够服务超过总线程数的任务！因为我们充分地利用了选手的等待时间，这样就极大
      * 地利用了有限的选手，完成了更多的击球动作，整体比赛效率因此得到提升。
      *
    ...
    /**
      ...
      * 模型其实就是一种特定的异步驱动模型。而也因此 AKKA 系统也被称为 SCALA 的协程架构。
      **/
}
