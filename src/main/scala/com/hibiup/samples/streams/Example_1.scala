...
object Example_1 {
    def HelloAkkaStream() = {
        ...
        /**
          ...
          * */
        def done: Future[Done] = source.runWith(Sink.foreach(println)) // 隐式获得 materializer 参数
...
object what_is_NotUsed extends App{
    def apply() = {
        ...
        def runnable: RunnableGraph[Future[String]] = source.toMat(sink)(Keep.right)
        def result: Future[String] = runnable.run()
